...
class MockCitationService {
  async parseResponseWithCitations(responseText: string, availableSources: string[]): Promise<ParsedResponse> {
  ...
  async validateCitations(responseText: string, availableSources: string[]): Promise<CitationValidationResult> {
    const errors: string[] = [];
    ...
    const claimIndicators = [
      ...
      'evidence shows', 'findings reveal', 'analysis demonstrates'
    ];
  ...
  async generateFollowUpQuestions(
    ...
    switch (questionType) {
      case 'clarifying':
        ...
        if (response.includes('various') || response.includes('multiple') || response.includes('different')) {
...
describe('Citation Parsing and Follow-up Generation', () => {
  ...
  describe('Citation Parsing', () => {
    it('should parse numbered citations correctly', async () => {
      const responseText = 'Machine learning is a subset of AI [1]. Deep learning uses neural networks [2]. This approach requires training data [3].';
      const availableSources = ['AI Fundamentals Guide', 'Deep Learning Handbook', 'ML Algorithms Book'];
    ...
    it('should parse named citations correctly', async () => {
      const responseText = 'According to the [AI Fundamentals Guide], machine learning is important. The [Deep Learning Handbook] explains neural networks.';
    ...
    it('should handle mixed citation styles', async () => {
      const responseText = 'Machine learning [1] is important. According to [AI Fundamentals Guide], it enables automation.';
      const availableSources = ['AI Fundamentals Guide', 'Deep Learning Handbook'];
    ...
    it('should generate clean text without citations', async () => {
      const responseText = 'Machine learning [1] is a subset of AI [2]. This field [3] is rapidly growing.';
  ...
  describe('Citation Validation', () => {
    ...
    it('should warn about uncited claims', async () => {
      const responseText = 'Research shows that this is true. Studies indicate significant improvements.';
      const availableSources = ['Source 1'];

      const validation = await mockService.validateCitations(responseText, availableSources);

      expect(validation.warnings.length).toBeGreaterThan(0);
      expect(validation.warnings.some(warning => warning.includes('may need a citation'))).toBe(true);
    });
  });

  describe('Follow-up Question Generation', () => {
    it('should generate appropriate follow-up questions', async () => {
      const responseText = 'Machine learning is important. However, it has limitations. For example, it requires large datasets.';
  ...
  describe('Citation Content Extraction', () => {
    it('should extract content for cited sources', async () => {
      const responseText = 'Machine learning [1] is important. Deep learning [2] uses neural networks.';
  ...
  describe('Citation Formatting', () => {
    it('should format citations in numbered style', async () => {
      const citations = [
        { id: '1', source: 'Source B', number: 2, content: '[2]', confidence: 0.9, startIndex: 0, endIndex: 3 },
        { id: '2', source: 'Source A', number: 1, content: '[1]', confidence: 0.9, startIndex: 0, endIndex: 3 }
      ];
    ...
    it('should format citations in alphabetical style', async () => {
      const citations = [
        { id: '1', source: 'Zebra Book', content: '[1]', confidence: 0.9, startIndex: 0, endIndex: 3 },
        { id: '2', source: 'Alpha Guide', content: '[2]', confidence: 0.9, startIndex: 0, endIndex: 3 }
      ];
    ...
    it('should format citations in author-date style', async () => {
      const citations = [
        { id: '1', source: 'Smith et al. (2023)', content: '[1]', confidence: 0.9, startIndex: 0, endIndex: 3 },
        { id: '2', source: 'Johnson Research', content: '[2]', confidence: 0.9, startIndex: 0, endIndex: 3 }
      ];
