...
export const FollowUpRequestSchema = z.object({
  ...
  questionType: z.enum(['clarifying', 'exploratory', 'deep-dive', 'related']).optional().default('exploratory'),
});
...
function generateFollowUpQuestions(
  originalQuery: string,
  responseText: string,
  questionType: string = 'exploratory',
  ...
  switch (questionType) {
    case 'clarifying':
      if (responseText.includes('might') || responseText.includes('could') || responseText.includes('possibly')) {
        questions.push('Can you provide more specific information about this topic?');
      }
      if (responseText.includes('various') || responseText.includes('multiple') || responseText.includes('different')) {
        questions.push('What are the specific examples or types you mentioned?');
      }
      if (responseText.includes('generally') || responseText.includes('typically') || responseText.includes('usually')) {
  ...
  if (responseText.toLowerCase().includes('however') || responseText.toLowerCase().includes('but')) {
    questions.push('What are the contrasting viewpoints on this topic?');
  }

  if (responseText.toLowerCase().includes('example') || responseText.toLowerCase().includes('such as')) {
    questions.push('Can you provide more specific examples?');
  }

  if (responseText.toLowerCase().includes('important') || responseText.toLowerCase().includes('significant')) {
...
function validateCitations(responseText: string, availableSources: string[]): CitationValidationResult {
  const errors: string[] = [];
  ...
  for (const match of numberedMatches) {
    ...
    if (number > availableSources.length) {
      errors.push(`Citation [${number}] refers to non-existent source (only ${availableSources.length} sources available)`);
    }
    if (number <= 0) {
  ...
  for (const match of namedMatches) {
    ...
    if (!/^\d+$/.test(sourceName) && !availableSources.includes(sourceName)) {
      ...
      const similar = availableSources.find(source => 
  ...
  const claimIndicators = [
    ...
    'evidence shows', 'findings reveal', 'analysis demonstrates', 'experts say'
  ];
...
function formatCitationsForDisplay(citations: Citation[], style: 'numbered' | 'alphabetical' | 'author-date' = 'numbered'): string[] {
...
async function parseResponseWithCitations(
  ...
  try {
    console.log(`Parsing citations from response with ${availableSources.length} available sources`);
    ...
    const uniqueCitations = allCitations.filter((citation, index, self) =>
      index === self.findIndex(c => c.source === citation.source)
    );
    ...
    console.log(`Parsed ${result.citationCount} citations and generated ${result.followUpQuestions.length} follow-up questions`);
    
    return result;

  } catch (error) {
    console.error('Error parsing citations:', error);
    throw new Error(`Citation parsing failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
async function generateFollowUpQuestionsFromContext(request: FollowUpRequest): Promise<string[]> {
  try {
    console.log(`Generating ${request.questionType} follow-up questions for query: "${request.originalQuery}"`);
    ...
    throw new Error(`Follow-up generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export const parseCitations = api(
  ...
  async (req: { responseText: string; availableSources: string[] }): Promise<ParsedResponse> => {
    try {
      ...
      console.log(`Parsing citations from response with ${validatedReq.availableSources.length} sources`);
      
      return await parseResponseWithCitations(validatedReq.responseText, validatedReq.availableSources);
      
    } catch (error) {
      console.error("Parse citations endpoint error:", error);
      throw new Error(`Failed to parse citations: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
);
...
export const validateCitationsEndpoint = api(
  ...
  async (req: { responseText: string; availableSources: string[] }): Promise<CitationValidationResult> => {
    try {
      // Validate request
      const validatedReq = ValidationRequestSchema.parse(req);
      
      console.log(`Validating citations in response with ${validatedReq.availableSources.length} sources`);
      
      const validation = validateCitations(validatedReq.responseText, validatedReq.availableSources);
      
      console.log(`Validation result: ${validation.valid ? 'valid' : 'invalid'} (${validation.errors.length} errors, ${validation.warnings.length} warnings)`);
...
export const formatCitations = api(
  ...
  async (req: { citations: Citation[]; style?: 'numbered' | 'alphabetical' | 'author-date' }): Promise<{ formatted: string[]; style: string }> => {
    try {
      ...
      throw new Error(`Failed to format citations: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
);
...
export const extractCitedContent = api(
  ...
  async (req: { responseText: string; context: Array<{ id: string; content: string; source: string }> }): Promise<Array<{ citation: Citation; sourceContent: string }>> => {
    try {
      console.log(`Extracting cited content from response with ${req.context.length} context items`);
      ...
      throw new Error(`Failed to extract cited content: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
);
