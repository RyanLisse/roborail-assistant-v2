...
class MockVectorSearchService {
  ...
  private setupMockData() {
    ...
    this.embeddings.set(
      ...
      this.generateMockEmbedding([0.7, 0.3, 0.5, 0.8])
    );
...
describe("Vector Search with PGVector", () => {
  ...
  describe("Vector Similarity Search", () => {
    ...
    it("should respect similarity threshold", async () => {
      ...
      const highThresholdResponse =
        await mockSearchService.performVectorSearch(highThresholdRequest);
      const lowThresholdResponse = await mockSearchService.performVectorSearch(lowThresholdRequest);

      expect(highThresholdResponse.results.length).toBeLessThanOrEqual(
        lowThresholdResponse.results.length
      );
    ...
    it("should enforce user access control", async () => {
      ...
      expect(
        user1Response.results.every(
          (r) => r.documentID.startsWith("doc_1") || r.documentID.startsWith("doc_2")
        )
      ).toBe(true);
  ...
  describe("Search Integration Scenarios", () => {
    ...
    it("should provide semantic similarity beyond keyword matching", async () => {
      ...
      expect(response.results.length).toBeGreaterThan(0);
      expect(
        response.results.some((r) => r.content.toLowerCase().includes("artificial intelligence"))
      ).toBe(true);
    });
  });
});
