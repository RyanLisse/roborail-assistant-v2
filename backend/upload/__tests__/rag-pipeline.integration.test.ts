...
import { type EmbeddingRequest, generateEmbeddings } from "../embedding";
import { type ParseRequest, parseDocument } from "../parser";
...
describe("RAG Pipeline Integration Tests", () => {
  ...
  describe("Document Processing Pipeline Components", () => {
    it("should process document parsing and embedding generation", async () => {
      ...
      const [document] = await db.select().from(documents).where(eq(documents.id, testDocumentId));
