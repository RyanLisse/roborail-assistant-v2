...
async function updateProcessingStatus(request: StatusUpdateRequest): Promise<ProcessingStatus> {
  ...
  const status: ProcessingStatus = {
    ...
    overallStatus: request.status === 'failed' ? 'failed' : 
                   request.status === 'completed' && !nextStage ? 'completed' : 'processing',
    stages: {
...
async function queryProcessingStatuses(query: StatusQueryRequest): Promise<ProcessingStatusResponse> {
