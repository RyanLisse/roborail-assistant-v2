...
describe("Semantic Chunking and Embedding Service", () => {
  describe("Text Chunking", () => {
    it("should chunk document text into semantic segments", async () => {
      const request: ChunkingRequest = {
        documentId: "doc_123",
        extractedText:
          "This is a sample document with multiple paragraphs. Each paragraph should be analyzed for semantic meaning. The chunking algorithm should identify natural breaks in the content.",
        elements: [
          {
            type: "title",
  ...
  describe("Cohere Embedding Generation", () => {
    ...
    it("should batch multiple texts efficiently", async () => {
      const texts = new Array(10)
        .fill(0)
        .map((_, i) => `Text chunk number ${i + 1} with unique content.`);
  ...
  describe("Complete Chunking and Embedding Pipeline", () => {
    it("should process document through complete pipeline", async () => {
      const request: ChunkingRequest = {
        ...
        extractedText:
          "Introduction to Machine Learning. Machine learning is a subset of artificial intelligence. It involves training algorithms on data. These algorithms can then make predictions on new data.",
        elements: [
          ...
          {
            type: "text",
            content: "These algorithms can then make predictions on new data.",
            page: 1,
          },
        ],
        metadata: {
  ...
  describe("Error Handling", () => {
    it("should handle API failures gracefully", async () => {
      const request: ChunkingRequest = {
        ...
        elements: [{ type: "text", content: "This should trigger an API failure.", page: 1 }],
        metadata: { language: "en" },
      };
    ...
    it("should handle very large documents appropriately", async () => {
      ...
      const request: ChunkingRequest = {
        ...
        elements: [{ type: "text", content: veryLongText, page: 1 }],
        metadata: { language: "en" },
      };
  ...
  describe("Token Counting and Optimization", () => {
    ...
    it("should optimize chunk boundaries for semantic coherence", async () => {
      const request: ChunkingRequest = {
        ...
        extractedText:
          "Chapter 1: Introduction. This chapter covers the basics. Chapter 2: Advanced Topics. This chapter goes deeper into the subject matter.",
        elements: [
          {
            type: "text",
            content: "Chapter 1: Introduction. This chapter covers the basics.",
            page: 1,
          },
          {
            type: "text",
            content:
              "Chapter 2: Advanced Topics. This chapter goes deeper into the subject matter.",
            page: 1,
          },
        ],
        metadata: { language: "en" },
...
async function processDocumentChunking(request: ChunkingRequest): Promise<ChunkingResponse> {
  ...
  try {
    ...
    if (
      ...
      request.extractedText.trim().length === 0
    ) {
      throw {
        type: "INVALID_INPUT",
...
function createChunk(
  ...
  const pageNumber =
    elements.find((el) => content.includes(el.content.substring(0, 20)))?.page || 1;
...
async function processDocumentChunkingWithFailure(
