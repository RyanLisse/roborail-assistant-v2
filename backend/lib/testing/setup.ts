...
class MockDeepEvalClient {
  ...
  async evaluateRubric(
    ...
    if (rubricLower.includes("complete")) {
      const isComplete = text.length > 20 && text.includes(".") && !text.trim().endsWith(",");
      if (isComplete) {
        score += 0.2;
...
if (!deepeval) {
  console.warn(
    "DEEPEVAL_API_KEY not set. DeepEval dependent tests may be skipped or use mock evaluation."
  );
}
  ...
  async toMatchSemanticSimilarity(received: string, expected: string, threshold = 0.8) {
    ...
    try {
      ...
      return {
        pass,
        message: () =>
          `Expected semantic similarity >= ${threshold}, got ${similarity.toFixed(3)}.\nReceived: "${received}"\nExpected: "${expected}"`,
      };
    } catch (e: any) {
      return {
        pass: false,
