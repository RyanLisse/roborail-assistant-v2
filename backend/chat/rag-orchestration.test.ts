...
describe("RAG Agent Orchestration", () => {
  ...
  describe("RAG Workflow Orchestration", () => {
    it("should orchestrate complete RAG workflow", async () => {
      ...
      const mockResult = {
        ...
        citations: [
          { documentId: "doc-1", filename: "benefits.pdf", relevanceScore: 0.9 }
        ],
        metadata: {
          searchTime: 150,
          llmTime: 800,
  ...
  describe("Error Handling and Fallbacks", () => {
    it("should provide fallback responses when no documents found", () => {
      const fallbackResponse = generateFallbackResponse("no_documents", "What is machine learning?");
...
function detectQueryIntent(query: string) {
  const documentKeywords = ["document", "paper", "report", "file", "uploaded", "says", "according to"];
...
function extractKeyTerms(query: string): string[] {
  const stopWords = new Set(["the", "and", "or", "but", "in", "on", "at", "to", "for", "of", "with", "by", "what", "how", "is", "are", "does", "do"]);
...
function assembleContext(options: any) {
  ...
  return {
    documentContext: options.searchResults.slice(0, 5).map((r: any) => r.content).join(" "),
    conversationContext: options.conversationHistory.slice(-4).map((h: any) => h.content).join(" "),
    totalTokens,
    wasTruncated,
  };
}
...
function generateFallbackResponse(reason: string, query: string) {
  return {
    content: "I don't have specific documents to reference for this question. However, I can provide general information based on my training.",
    type: "fallback",
