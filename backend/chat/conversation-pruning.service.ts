...
import { type ContextOptionsType, pruneConversationHistory } from "./context-management";
...
export class ConversationPruningService {
   ...
   */
  async pruneConversation(messages: Message[], options: PruningOptions = {}): Promise<Message[]> {
    try {
      const {
        maxMessages = 20,
        maxTokens,
      ...
      const contextOptions: Partial<ContextOptionsType> = {
        maxMessages: maxMessages as any,
        maxContextChars: maxContextChars as any,
        maxContextTokens: (maxTokens || maxContextTokens) as any,
        minRecentMessages: minRecentMessages as any,
        prioritizeRecent,
      };

      const prunedMessages = pruneConversationHistory(conversationMessages as any, contextOptions);
