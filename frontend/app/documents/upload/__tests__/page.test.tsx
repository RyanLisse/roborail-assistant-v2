...
// Ensure you have @types/testing-library__jest-dom and @types/testing-library__react installed
/// <reference types="@testing-library/jest-dom" />
/// <reference types="@testing-library/react" />
import { describe, it, expect, vi } from 'vitest';
import '@testing-library/jest-dom/extend-expect'; // Ensure matchers are loaded
...
// @ts-expect-error: TypeScript may not recognize jest-dom matchers, but they are loaded at runtime
...
describe('DocumentUploadPage', () => {
  ...
  it('triggers mutation on valid submit', async () => {
    render(<DocumentUploadPage />);
    const file = new File(['dummy content'], 'test.pdf', { type: 'application/pdf' });
    const fileInput = screen.getByLabelText(/Document File/i) as HTMLInputElement;
    fireEvent.change(fileInput, { target: { files: [file] } });
    fireEvent.change(screen.getByLabelText(/Title/i), { target: { value: 'Test Title' } });
    fireEvent.change(screen.getByLabelText(/Source Type/i), { target: { value: 'application/pdf' } });
    fireEvent.click(screen.getByText(/Upload Document/i));
    // Mutation is mocked, so just check for loading/progress UI
    await waitFor(() => {
